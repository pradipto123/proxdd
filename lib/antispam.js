...
const addSpam = (nama, userId, expired, _data) => {
    ...
    _data.push(obj)
    global.setDatabase('antispam.json', JSON.stringify(_data))
}
    ...
    setInterval(() => {
        ...
        if (found !== false) {
            if (_db[found].expired === 'PERMANENT') {
                ...
                _db.splice(found, 1)
                global.setDatabase('antispam.json', JSON.stringify(_db))
            }
        }
    }, 1000)
}
...
global.msgFilter = { isFiltered, addFilter }
global.addSpam = addSpam
global.unSpam = unSpam
global.SpamExpired = SpamExpired
global.cekSpam = cekSpam
